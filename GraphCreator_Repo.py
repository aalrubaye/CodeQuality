...
def issues_in_range(myList):

    hr = 2

    start = numpy.mean(myList) - (hr * 3600)
    end = numpy.mean(myList) + (hr * 3600)
    issues_around_mean = len([i for i in myList if start < i < end])
    issues_below_mean = len([i for i in myList if start > i])
    issues_above_mean = len([i for i in myList if i > end])

    return [issues_below_mean,
            issues_around_mean,
            issues_above_mean]
...
def before_notification_graph_generator():
    ...
    issues_below_mean_related_repos = \
        [[], [], [], [], [], [], [], [], [], [],
         [], [], [], [], [], [], [], [], [], []]
    issues_around_mean_related_repos = \
        [[], [], [], [], [], [], [], [], [], [],
         [], [], [], [], [], []]
    issues_above_mean_related_repos = \
        [[], [], [], [], [], [], [], [], [], [],
         [], [], [], [], [], [], [], [], [], []]
        ...
        try:
            ...
            if count > 0:
                ...
                if main_entry.get('contributors_count'):
                    ...
                    dist = distribution(all_months_days_between_issues)
                    issues_count_in_range = issues_in_range(all_months_days_between_issues)

                    issues_below_mean = issues_count_in_range[0]
                    issues_around_mean = issues_count_in_range[1]
                    issues_above_mean = issues_count_in_range[2]
                    ...
                    if issues_count > 0:
                        ...
                        issues_below_prc = issues_below_mean / float(issues_count)
                        issues_around_prc = issues_around_mean / float(issues_count)
                        issues_above_prc = issues_above_mean / float(issues_count)

                        print  '['+ str(issues_below_prc)+' , '+str(issues_around_prc)+' , '+str(issues_above_prc)+'] -> '+ repo_name

                        # this will be based on the "percentage of the issues distances" pattern
                        j2 = [i for i in issue_range_below if i >= issues_below_prc]
                        below_index = len(issue_range_below) - len(j2)
                        issues_below_mean_related_repos[below_index].append(repo_name)

                        # this will be based on the "percentage of the issues distances" pattern
                        j2 = [i for i in issue_range_around if i >= issues_around_prc]
                        around_index = len(issue_range_around) - len(j2)
                        issues_around_mean_related_repos[around_index].append(repo_name)

                        # above
                        j2 = [i for i in issue_range_above if i >= issues_above_prc]
                        above_index = len(issue_range_above) - len(j2)
                        issues_above_mean_related_repos[above_index].append(repo_name)

                        graph.add_node(repo_name, name=repo_name, lang=lang, group_below= below_index, group_around=around_index, group_above=above_index)
    ...
    #     entry = issues_around_mean_related_repos[i]
    #     for k in range(0, len(entry)):
    #         issues_around_mean_related_repos[19].append(entry[k])
    #     if i < 19:
    #         issues_around_mean_related_repos[i] = []

    # # edges between issue related repos - below mean
    # for entry in issues_below_mean_related_repos:
    ...
    # edges between issue related repos - around mean
    for entry in issues_around_mean_related_repos:
        for i in range(0, len(entry)):
            for j in range(i+1, len(entry)):
                node1 = entry[i]
                node2 = entry[j]
                if node1 != node2:
                    if graph.has_edge(node1, node2):
                        ...
                        graph.add_edge(node1, node2, weight=1)
    #
    # # edges between issue related repos - above mean
    # for entry in issues_above_mean_related_repos:
    #     for i in range(0, len(entry)):
    #         for j in range(i+1, len(entry)):
    #             node1 = entry[i]
    #             node2 = entry[j]
    ...
    nx.write_graphml(graph, "/Users/Abduljaleel/Desktop/1hr-issues_before.graphml")
...
def after_notification_graph_generator(prob):
        ...
        try:
            ...
            if count > 0:
                ...
                if main_entry.get('contributors_count'):
                    ...
                    issues_count_in_range = issues_in_range(final_list_after_simulator)
                    ...
                    if issues_count > 0:
                        ...
                        print repo_name + ' -> ['+ str(issues_below_prc)+' , '+str(issues_around_prc)+' , '+str(issues_above_prc)+']'
    ...
    nx.write_graphml(graph, "/Users/Abduljaleel/Desktop/1hr-Issues"+str(prob)+"_after.graphml")


if __name__ == "__main__":
    graph = nx.Graph()
    before_notification_graph_generator()
    # after_notification_graph_generator(0.3)
